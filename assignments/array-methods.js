// A local community center holding a fund raising 5k fun run has invited 50 small businesses to make a small donation on their behalf for some much needed updates to their facilities.  Each business has assigned a representative to attend the event along with a small donation.
...
// What I need is a map that returns two values from different keys at each index.
// the dot notation version is element.key, so I want element.first_name and element.last_name;
// Also I want to use backtick notation for the names.
// So `${first_name} ${last_name}`
// let fullName = runners.map(names => {return `${names.first_name} ${names.last_name}`});
// Logging with a join to not have my node scroll horribly.
// console.log('Challenge 1 Answer: ' + fullName.join(','));

// OH WHOOPS SHOULD HAVE USED FOR EACH k, k, I can fix this.
let fullName = [];
runners.forEach((names) => {fullName.push(`${names.first_name} ${names.last_name}`)});
// Logging with a join to not have my node scroll horribly.
console.log('Challenge 1 Answer: ' + fullName.join(','));
...
// Similar thought process engage: .map, pull each first_name, .toUpperCase it.
// backticks are more optional here but I'm using them so I get comfortable.
let allCaps = runners.map((NAME) => {return `${NAME.first_name.toUpperCase()}`});
// Logging with a join to not have my node scroll horribly.
console.log('Challenge 2 Answer: ' + allCaps.join(','));
...
// tried to construct an if function like the MDN example?
// {
//   if (backorder.shirt_size === 'L') {
//   return `Name: ${backorder.first_name} ${backorder.last_name}, email: ${backorder.email}`
// }});
// console.log('Challenge 3 Answer: ' + largeShirts);
console.log('Challenge 3 Answer: ' + JSON.stringify(largeShirts));

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce(function(ticketPriceTotal,currentValue) {return ticketPriceTotal + currentValue.donation},0);
console.log('Challenge 4 Answer: ' + ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
let highestDonations = runners.filter(heroes => {return heroes.donation > 200});
console.log(highestDonations);

// Problem 2
let spamEmail = runners.filter(donators => {return donators.donation < 50}).map(nag => {return `To: ${nag.email}; Hey ${nag.first_name}, have you considered pestering every last person on your social media accounts to throw more money at us?`})
console.log(spamEmail);

// Problem 3
let hungryForNoodles = runners.filter(noodler => {return noodler.company_name.match(/noodl/)}).map(noodlcart => {return noodlcart.company_name});
console.log('Let\'s order from a sponsoring noodle vendor: ' + hungryForNoodles.join(', '));
